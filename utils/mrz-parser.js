...
const { fromTriangles } = require("transformation-matrix");
...
function customParser(text) {
  // text = "P<UZBSAGDIEV<<ISKANDER<<<<<<<<<<<<<<<<<<<<<<\nAA48803533U2B9310298M24032813291093020001288\n"
  try {
    var lines = text.split("\n");

    var names = lines[0].slice(5).split("<<");
    names = _stripPadding(names);

    var documentNumber = _stripPadding(lines[1].substring(0, 9));

    var dob = _getFullDate(lines[1].slice(13, 19));

    var sex = _getSex(lines[1].substring(20, 21));

    var expiry = _getFullDate(lines[1].substring(21, 27));

    var personalNumber = _stripPadding(lines[1].slice(28, 42));
    ...
    return {
      ...
      "sex": sex.abbr,
      "formattedDateOfExpiry": `${expiry.day}.${expiry.month}.${expiry.year}`,
      "personalNumber": personalNumber,
    };
  } catch (err) {
    throw err;
...
function idParse(mrz) {
  ...
  try {
    var issuerOrg = _stripPadding(mrz.substring(2, 5));
    var names = _getNames(mrz.substring(60, mrz.length));
    var documentNumberRaw = mrz.substring(5, 14);
    var documentNumber = _stripPadding(documentNumberRaw);
    var checkDigit1 = mrz.substring(14, 15);
    var checkDigitVerify1 = _checkDigitVerify(documentNumberRaw, checkDigit1);
    var nationality = _getNationality(_stripPadding(mrz.substring(48, 51)));
    var dobRaw = mrz.substring(30, 36);
    var dob = _getFullDate(_stripPadding(dobRaw));
    var checkDigit2 = mrz.substring(36, 37);
    var checkDigitVerify2 = _checkDigitVerify(dobRaw, checkDigit2);
    var sex = _getSex(_stripPadding(mrz.substring(37, 38)));
    var expiryRaw = mrz.substring(38, 44);
    var expiry = _getFullDate(_stripPadding(expiryRaw));
    var checkDigit3 = _stripPadding(mrz.substring(44, 45));
    var checkDigitVerify3 = _checkDigitVerify(expiryRaw, checkDigit3);
    var personalNumberRaw = mrz.substring(15, 29);
    var personalNumber = _stripPadding(personalNumberRaw);
    ...
    return {
      ...
      checkDigit: {
        ...
        dob: { value: checkDigit2, valid: checkDigitVerify2 },
        expiry: { value: checkDigit3, valid: checkDigitVerify3 },
        valid: checkDigitVerify1 && checkDigitVerify2 && checkDigitVerify3
      },
      expiry: expiry,
      personalNumber: personalNumber
    };
...
function idCardParser(text) {
  // IUUZBAD0195745452512345678912<\n0408259M3123456XXXUZB<<<<<<<<0\nBAHODIROV<<OTABEKK<<<<<<<<<<<<\n
  // P<UZBSAGDIEV<<ISKANDER<<<<<<<<<<<<<<<<<<<<<<\nAA48803533U2B9310298M24032813291093020001288\n
  // IUUZBAD0195745452508048680028<\n0408259M3102259XXXUZB<<<<<<<<0\nBAHODIROV<<ASADBEK<<<<<<<<<<<<\n
  try {
    var lines = text.split("\n");

    var names = lines[2].slice(0).split("<<");
    names = _stripPadding(names);

    var documentNumber = _stripPadding(lines[0].substring(5, 14));

    var dob = _getFullDate(lines[1].slice(0, 6));

    var sex = _getSex(lines[1].substring(7, 8));

    var expiry = _getFullDate(lines[1].substring(8, 14));

    var personalNumber = _stripPadding(lines[0].slice(15, 29));
